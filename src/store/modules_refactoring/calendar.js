...
const actions = {
  async add_schedule({ dispatch, rootGetters }, form) {
    const apiClient = rootGetters["global/apiClient"];
    const { success, error, response } = await apiClient.schedule.createSchedule(form);
  ...
  async select_schedule({ commit, dispatch, rootGetters }) {
    const apiClient = rootGetters["global/apiClient"];
    const { success, error, response } = await apiClient.schedule.getScheduleList();

    if (success) {
      commit("SET_SCHEDULE_LIST", response.data);
      return response.data;
    } else {
      dispatch("apiErrorHandler", { error }, { root: true });
      return false;
  ...
  async get_schedule_info({ commit, dispatch, rootGetters }, id) {
    const apiClient = rootGetters["global/apiClient"];
    const { success, error, response } = await apiClient.schedule.getInfo(id);

    if (success) {
      commit("GET_SCHEDULE_INFO", response.data);
      return response.data;
    } else {
      dispatch("apiErrorHandler", { error }, { root: true });
      return false;
  ...
  async delete_schedule({ dispatch, rootGetters }, schedule_id_form) {
    const apiClient = rootGetters["global/apiClient"];
    const { success, error, response } = await apiClient.schedule.deleteSchedule(schedule_id_form);
  ...
  async update_schedule({ dispatch, rootGetters }, update_form) {
    const apiClient = rootGetters["global/apiClient"];
    const { success, error, response } = await apiClient.schedule.updateSchedule(update_form);
  ...
  async get_member_squad_info({ dispatch, rootGetters }, form) {
    const apiClient = rootGetters["global/apiClient"];
    const { success, error, response } = await apiClient.schedule.getMemberSquadInfo(form.year, form.month);
