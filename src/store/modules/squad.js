...
const mutations = {
    divide_member_into_team(state, {
        ...
        if (isInputErrorExist(attend_member_id_list,
                number_team,
                min_player,
                jocker_exist)) {
            return false;
        ...
        return true
        // 위에 필요한 함수
        function isInputErrorExist(attend_member_id_list,
            number_team,
            min_player,
            jocker_exist) {

            let needed_number_of_player = 0
            if (jocker_exist) {
                needed_number_of_player = number_team * min_player + 1
            } else {
                needed_number_of_player = number_team * min_player
            }
            return attend_member_id_list.length < needed_number_of_player
        }
