...
test("Get Attend Rate For 3 month with Last Date", async () => {
    ...
    // subset이 존재하는지 확인
    expect(store.state.good_attend).toEqual(expect.arrayContaining(expected_subset_good_attend))
    expect(store.state.so_so_attend).toEqual(expect.arrayContaining(expected_subset_so_so_attend))
    expect(store.state.ghost_attend).toEqual(expect.arrayContaining(expected_subset_ghost_attend))
